...
|------|---------------|---------|-----------|------|
| 盘前 | 09:25 | 01:25 | `25 1 * * 1-5` | 开盘前最后检查（intraday.yml） |
| 上午盘中 | 10:00, 10:30 | 02:00, 02:30 | `0,30 2 * * 1-5` | 上午行情跟踪（intraday.yml） |
| 下午盘中 | 13:00, 13:30 | 05:00, 05:30 | `0,30 5 * * 1-5` | 下午行情跟踪（intraday.yml） |
| 尾盘 | 14:50 | 06:50 | `50 6 * * 1-5` | **关键窗口**：尾盘信号（intraday.yml） |
| 收盘后 | 15:30 | 07:30 | `30 7 * * 1-5` | 完整日线确认（daily.yml） |
| 晚间 | 19:00 | 11:00 | `0 11 * * 1-5` | 复盘汇总（daily.yml） |

**合计 7 次/工作日**（原设计 13 次/天）。

**优化原因（实测）**：
- GitHub 免费 runner 在美东下午（BJT 凌晨 0-4 点）为高峰，job 排队 15 分钟以上会因 "job was not acquired" 失败
- BJT 9:25-19:00（= UTC 1:25-11:00 = 美东晚间至清晨）为低峰时段，正好覆盖 A 股交易时段
- 减少触发次数降低 runner 占用，排队概率大幅下降
- 盘前数据在 9:25 拉取与 9:30 开盘前数据差异极小，精简为单次

**注意**：GitHub Actions cron 使用 UTC 时间。北京时间 = UTC + 8 小时。

**实现说明**：
- 盘中高频触发会导致信号频繁变化，因此盘中信号（intraday.yml）与日终报告（daily.yml）分开配置
- 每个 workflow 支持 `workflow_dispatch` 手动触发（用于补跑/测试）
- 两个 workflow 均设置 `timeout-minutes: 15`（正常执行约 3-5 分钟）
